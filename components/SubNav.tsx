...
const SingleVariantChartItem = ({ chart }: { chart: ChartSchema }) => {
  ...
  return (
    <Link passHref key={`subnav-chart-${chart.slug}`} href={linkTo(`/charts/${chart.slug}`)}>
      <NavLink
        className="my-3 rounded py-1 px-2 font-medium text-gray-800 transition first:-ml-2 last:-mr-2 hover:bg-gray-800 hover:text-white"
        activeClassName="!text-gray-800 bg-white hover:!bg-white"
      >
...
const MenuButton = ({ children, isActive }: { children: React.ReactNode; isActive: boolean }) => {
  return (
    <Menu.Button className="group my-3 flex items-center rounded py-1 px-2 font-medium text-gray-800 transition first:-ml-2 hover:bg-gray-800 hover:text-white">
      <span
